    ...
    <bg-img>
        ...
        <div class="w-full mx-auto sm:w-11/12 md:w-8/12 lg:w-1/2 max-w-xl py-16">
            <form @submit.prevent="submit" class="mx-8 px-8 py-8 bg-gray-50 bg-opacity-90 rounded shadow-md">
                ...
                <div class="mt-4">
                    <breeze-label for="account_type" value="Account Type" />
                    <select v-model="form.account_type" required class="mt-1 block w-full text-gray-600 border-gray-300 focus:border-green-300 focus:ring focus:ring-green-200 focus:ring-opacity-50 rounded-md shadow-sm py-3">
                        <option value="" disabled>Select Account Type</option>
                        <option :value="account_type" v-for="account_type in account_types" :key="account_type" class="">{{ account_type }}</option>
                    </select>
                </div>
